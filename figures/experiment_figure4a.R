...
fn = sprintf("../experiments/experiment_LpRR_convergence_noiseless.mat");
...
tex_path = sprintf("../../61f87e85d1b475ca03d2c008/figures/experimentLpRR_convergence_noiseless.tex");
...
p = ggplot(data=dat, mapping=aes(x=num_iters,y=errors, fill=groups,shape=groups)) +
     ...
     scale_y_continuous(trans="log10", breaks=c(1, 1e-5, 1e-10, 1e-16),
                                       labels=c("1", "$10^{-5}$", "$10^{-10}$", "$10^{-16}$") ) +
     ...
     theme(axis.text=element_text(face="plain", size=rel(0.8), color='gray10'),
         ...
         legend.position=c(0.22,0.32), # legend.position = "none",
